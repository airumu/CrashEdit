...
namespace CrashEdit.Crash
{
    public static class SF2Conv
    {
        ...
        private static List<KeyValuePair<int, VHProgram>> vhPrograms = new();
        ...
        public static byte[] ToSF2(VAB vab)
        {
            ...
            vhPrograms = [.. vh.Programs];
            vhPrograms.Sort((a, b) => a.Key.CompareTo(b.Key));
            ...
            // Generate PCM from SampleSet and set the loop flags.
        ...
        private static List<int> ComputeInstZoneOffsets()
        {
            var programs = vhPrograms;
            List<int> offsets = new List<int>();
            int offs = 0;
            foreach (var prog in vhPrograms)
            {
                offsets.Add(offs);
        ...
        private static void WriteINFOChunk(BinaryWriter bw)
        {
            ...
            using (BinaryWriter bwSub = new BinaryWriter(msSub))
            {
                ...
                WriteStringSubChunk(bwSub, "ISFT", $"CrashEdit v0.4.0.2");
        ...
        private static void WritePdtaChunk(BinaryWriter bw, int sampleRate)
        {
            using (MemoryStream msList = new MemoryStream())
            using (BinaryWriter bwList = new BinaryWriter(msList))
            {
        ...
        /// </summary>
        private static byte[] CreatePhdrChunk()
        {
            var programs = vhPrograms;
        ...
        /// </summary>
        private static byte[] CreatePbagChunk()
        {
            var programs = vhPrograms;
            int presetCount = programs.Count;
            using (MemoryStream ms = new MemoryStream())
        ...
        /// </summary>
        private static byte[] CreatePgenChunk()
        {
            var programs = vhPrograms;
            int presetCount = programs.Count;
            using (MemoryStream ms = new MemoryStream())
        ...
        /// </summary>
        private static byte[] CreateInstChunk()
        {
            var programs = vhPrograms;
            var instZoneOffsets = ComputeInstZoneOffsets();
            int presetCount = programs.Count;
            using (MemoryStream ms = new MemoryStream())
            using (BinaryWriter bw = new BinaryWriter(ms))
            {
        ...
        /// </summary>
        private static byte[] CreateIbagChunk()
        {
            var programs = vhPrograms;
            int presetCount = programs.Count;
            using (MemoryStream ms = new MemoryStream())
        ...
        /// </summary>
        private static byte[] CreateIgenChunk()
        {
            var programs = vhPrograms;
            using (MemoryStream ms = new MemoryStream())
            using (BinaryWriter bw = new BinaryWriter(ms))
