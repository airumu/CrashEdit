namespace CrashEdit.Crash
{
    public sealed class ZoneHeader : IResource
    {
        public static ZoneHeader Load(byte[] data)
        {
            ...
            return new ZoneHeader(data, worldCount, worlds, infoCount, cameraCount, entityCount, zoneCount, zones, zoneLinkTypes, chunk1, zoneFlags, unk0x2A0, music, chunk2);
        }

        public ZoneHeader(byte[] data, int worldCount, List<int> worlds, int infoCount, int cameraCount, int entityCount, int zoneCount, List<int> zones, List<int> zoneLinkTypes, byte[] chunk1, int zoneFlags, int unk0x2A0, int music, byte[] chunk2)
        {
            Data = data;
