namespace CrashEdit.Crash
{
    public sealed class VHTone
    {
        ...
        public RIFF ToDLSCreatergn2(bool drumkit)
        {
            ...
            BitConv.ToInt16(rgnh, 4, 0);         // usVelLow
            BitConv.ToInt16(rgnh, 6, 127);       // usVelHigh
            BitConv.ToInt16(rgnh, 8, 1);         // fusOptions
            BitConv.ToInt16(rgnh, 10, 0);        // usKeyGroup
            BitConv.ToInt16(rgnh, 12, 1);        // DLS2
            rgn.Items.Add(new RIFFData("rgnh", rgnh));

            // wsmp
            byte[] wsmp = new byte[24];
            BitConv.ToInt32(wsmp, 0, 20);                  // cbSize (size of structure without loop record)
            BitConv.ToInt16(wsmp, 4, CenterNote);           // Unity Note
            BitConv.ToInt16(wsmp, 6, PitchShift);           // Fine Tune
            BitConv.ToInt32(wsmp, 8, (Volume - 64) << 16);   // lAttenuation
            BitConv.ToInt32(wsmp, 12, 1);                   // fulOptions
            BitConv.ToInt32(wsmp, 16, 0);                   // reserved
            BitConv.ToInt32(wsmp, 20, 0);                   // set 0 if no loop
            ...
            int channel = drumkit ? 3 : 0; // melodic patch ¨ channel 0
            BitConv.ToInt32(wlnk, 4, channel);
            int tableIndex = (Wave > 0) ? Wave - 1 : 0;
            BitConv.ToInt32(wlnk, 8, tableIndex);
