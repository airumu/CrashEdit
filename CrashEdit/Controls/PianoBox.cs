...
namespace M
{
    ...
    public class PianoBox : Control
    {
        ...
        static readonly double _BlackKeyHeight = .4;
        static readonly int _TextOffset = 20;
            ...
            using (var selBrush = new SolidBrush(_noteHighlightColor))
            {
                if (Orientation.Horizontal == _orientation)
                {
                    ...
                    for (var i = 1; i < whiteKeyCount; ++i)
                    {
                        ...
                        {
                            if (_keys[key])
                                g.FillRectangle(selBrush, ox + 1, 1, wkw - 1, Height - _TextOffset);

                            ++key;
                            if (_keys[key])
                                g.FillRectangle(selBrush, x - (bkw / 2) + 1, 1, bkw - 1, unchecked((int)(Height * _BlackKeyHeight)));
                            ++key;
                            if (_keys[key])
                                g.FillRectangle(selBrush, x, 1, wkw - 1, Height - _TextOffset);
                        }
                        else
                        {
                            if (_keys[key])
                    ...
                    // draw the text at the bottom
                    {
                        ...
                        {
                            int keyWidth = Width / whiteKeyCount;
                            int keyIndex = 0;
                            for (int octave = 0; octave < _octaves; octave++)
                            {
                                for (int i = 0; i < 7; i++)
                                {
                                    int midiNumber = startMidi + octave * 12 + whiteKeyOffsets[i];
                                    if (midiNumber == octave * 12)
                                    {
                                        int x = keyIndex * keyWidth;
                                        int y = Height - font.Height - 2;
                                        SizeF textSize = g.MeasureString(midiNumber.ToString(), font);
                                        int textX = x + (keyWidth - (int)textSize.Width) / 2;
                                        g.DrawString(midiNumber.ToString(), font, textBrush, textX, y);
                                    }
                                    keyIndex++;
                                }
                            }
                    ...
                    using (var pen = new Pen(_borderColor))
                    {
                        ...
                        if (Orientation.Horizontal == _orientation)
                        {
                            ...
                            for (var i = 1; i < whiteKeyCount; ++i)
                            {
                                ...
                                if (3 != k && 0 != k)
                                {
                                    g.DrawRectangle(pen, x - (bkw / 2), 0, bkw, unchecked((int)(Height * _BlackKeyHeight)) + 1);
                                    ++key;
                                    if (!_keys[key])
                                        g.FillRectangle(brush, x - (bkw / 2) + 1, 1, bkw - 1, unchecked((int)(Height * _BlackKeyHeight)));
                                    g.DrawLine(pen, x, 1 + unchecked((int)(Height * _BlackKeyHeight)), x, Height - _TextOffset);
                                    ++key;
                                }
                                else
                                {
                                    g.DrawLine(pen, x, 1, x, Height - _TextOffset);
                                    ++key;
                                }
                                if (i > 73) break;
                            }
                        ...
                        else // vertical
                        {
                            ...
                            for (var i = 1; i < whiteKeyCount; ++i)
                            {
                                ...
                                if (4 != k && 0 != k)
                                {
                                    g.DrawRectangle(pen, 0, y - (bkh / 2), unchecked((int)(Width * _BlackKeyHeight)), bkh - 1);
                                    --key;
                                    if (!_keys[key])
                                        g.FillRectangle(brush, 1, y - (bkh / 2) + 1, unchecked((int)(Width * _BlackKeyHeight)) - 1, bkh - 2);
                                    g.DrawLine(pen, 1 + unchecked((int)(Width * _BlackKeyHeight)), y, Width - 2, y);
                                    --key;
                                }
                                else
                                {
        ...
        int _HitTest(int x, int y)
        {
            ...
            if (Orientation.Horizontal == Orientation)
            {
                var wkw = Width / whiteKeyCount;
                var oct = (x / wkw) / 7;
                // lower side of black key height
                if (y > Height * _BlackKeyHeight)
                {
                    switch ((x / wkw) % 7)
                    {
                        case 0:
                ...
                // upper side of black key height (including the white keys)
                else
                {
            ...
            else // vertical
            {
                ...
                {
                    var bkh = unchecked((int)(wkh * _BlackKeyHeight));
                    switch ((y / wkh) % 7)
                    {
                        case 0:
